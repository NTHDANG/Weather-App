...
const ForecastDisplay = ({ forecastData }) => {
  ...
  return (
    ...
    >
      <h3 className="text-4xl font-bold mb-5 flex items-center justify-center text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-600 drop-shadow-lg">
        Dự báo 5 ngày tới
      </h3>
      <div className="flex flex-wrap justify-center gap-4 mt-10 text-xl">
            ...
            {Object.entries(dayData.times).map(([timeOfDay, item], index) => (
              <React.Fragment key={timeOfDay}>
                <div className="text-center mb-2 pb-2">
                  <p className="text-lg font-bold">{timeOfDay}</p>
                  <img
                    src={`https://openweathermap.org/img/wn/${item.weather[0].icon}.png`}
                    alt={item.weather[0].description}
                    className="w-15 h-15 mx-auto"
                  />
                  <p className="text-lg">{Math.floor(item.main.temp)}°c</p>
